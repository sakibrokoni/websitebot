...
(function ($) {
  ...
  function selectOption($this, container, data, delay) {
    $this.parent().hide();
    var $userReply = $(
      '<li class="user"><div class="text">' + $this.html() + "</div></li>"
    );
  ...
  function startChat(container, data, startId, delay) {
    // clear chat window
    container.html(
      "<div class='footer-chat'>WeConnect <sup>Powered By Intelsense Ai</sup></div>"
    );
  ...
  function addOptions(container, data, delay, m) {
    ...
    for (var i = 1; i < 12; i++) {
      ...
      if (optionText != "" && optionText != undefined && optionText != null) {
        // add option only if text exists
        var $optionElem = $(
  ...
  function toggleLoader(status, container) {
    ...
    else container.find(".typing-indicator").remove();
  }
